    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="!max-w-[90%] w-full max-h-[85vh] overflow-hidden flex flex-col">
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4 shrink-0">
            <div className="flex items-center gap-2">
              <span className="rounded-full border border-red-500/30 bg-red-500/10 px-2 py-0.5 text-[10px] font-medium uppercase tracking-wider text-red-400">{t("pcomponents_DocumentDiffModal.old", {defaultValue: "Old"})}</span>
              <Select
              ...
              >
                <SelectTrigger className="h-7 w-60 text-xs border-border/60">
                  <SelectValue placeholder={t("pcomponents_DocumentDiffModal.select_revision", {defaultValue: "Select revision"})} />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="flex items-center gap-2">
              <span className="rounded-full border border-green-500/30 bg-green-500/10 px-2 py-0.5 text-[10px] font-medium uppercase tracking-wider text-green-400">{t("pcomponents_DocumentDiffModal.new", {defaultValue: "New"})}</span>
              <Select
        ...
        <div className="overflow-auto flex-1 rounded-md border border-border text-xs">
          {!revisions ? (
            <div className="p-6 text-center text-muted-foreground text-sm">{t("pcomponents_DocumentDiffModal.loading_revisions", {defaultValue: "Loading revisions..."})}</div>
          ) : !leftRevision || !rightRevision ? (
            <div className="p-6 text-center text-muted-foreground text-sm">{t("pcomponents_DocumentDiffModal.select_two_revisions_to_compare", {defaultValue: "Select two revisions to compare."})}</div>
          ) : leftRevision.id === rightRevision.id ? (
            <div className="p-6 text-center text-muted-foreground text-sm">{t("pcomponents_DocumentDiffModal.both_sides_are_the_same_revision", {defaultValue: "Both sides are the same revision."})}</div>
          ) : (
            <div className="font-mono text-[12px] leading-6">
              <div className="grid grid-cols-[56px_56px_24px_minmax(0,1fr)] border-b border-border/60 bg-muted/30 px-3 py-2 text-[11px] uppercase tracking-wide text-muted-foreground">
                <span>{t("pcomponents_DocumentDiffModal.old", {defaultValue: "Old"})}</span>
                <span>{t("pcomponents_DocumentDiffModal.new", {defaultValue: "New"})}</span>
                <span />
                <span>{t("pcomponents_DocumentDiffModal.content", {defaultValue: "Content"})}</span>
              </div>
