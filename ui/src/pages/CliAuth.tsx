...
export function CliAuthPage() {
  ...
  if (!challengeId || !token) {
    return <div className="mx-auto max-w-xl py-10 text-sm text-destructive">{t("ppages_CliAuth.invalid_cli_auth_url", {defaultValue: "Invalid CLI auth URL."})}</div>;
  }

  if (sessionQuery.isLoading || challengeQuery.isLoading) {
    return <div className="mx-auto max-w-xl py-10 text-sm text-muted-foreground">{t("ppages_CliAuth.loading_cli_auth_challenge", {defaultValue: "Loading CLI auth challenge..."})}</div>;
  }
    ...
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          <h1 className="text-lg font-semibold">{t("ppages_CliAuth.cli_auth_challenge_unavailable", {defaultValue: "CLI auth challenge unavailable"})}</h1>
  ...
  if (!challenge) {
    return <div className="mx-auto max-w-xl py-10 text-sm text-destructive">{t("ppages_CliAuth.cli_auth_challenge_unavailable", {defaultValue: "CLI auth challenge unavailable."})}</div>;
  }
  ...
  if (challenge.status === "approved") {
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          <h1 className="text-xl font-semibold">{t("ppages_CliAuth.cli_access_approved", {defaultValue: "CLI access approved"})}</h1>
          <p className="mt-2 text-sm text-muted-foreground">
  ...
  if (challenge.requiresSignIn || !sessionQuery.data) {
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          <h1 className="text-xl font-semibold">{t("ppages_CliAuth.sign_in_required", {defaultValue: "Sign in required"})}</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            Sign in or create an account, then return to this page to approve the CLI access request.
          </p>
          <Button asChild className="mt-4">
            <Link to={`/auth?next=${encodeURIComponent(currentPath)}`}>{t("ppages_CliAuth.sign_in_create_account", {defaultValue: "Sign in / Create account"})}</Link>
          </Button>
        </div>
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        <h1 className="text-xl font-semibold">{t("ppages_CliAuth.approve_paperclip_cli_access", {defaultValue: "Approve Paperclip CLI access"})}</h1>
        <p className="mt-2 text-sm text-muted-foreground">
        ...
        <div className="mt-5 space-y-3 text-sm">
          <div>
            <div className="text-muted-foreground">{t("ppages_CliAuth.command", {defaultValue: "Command"})}</div>
            <div className="font-mono text-foreground">{challenge.command}</div>
          </div>
          <div>
            <div className="text-muted-foreground">{t("ppages_CliAuth.client", {defaultValue: "Client"})}</div>
            <div className="text-foreground">{challenge.clientName ?? "paperclipai cli"}</div>
          </div>
          <div>
            <div className="text-muted-foreground">{t("ppages_CliAuth.requested_access", {defaultValue: "Requested access"})}</div>
          ...
          {challenge.requestedCompanyName && (
            <div>
              <div className="text-muted-foreground">{t("ppages_CliAuth.requested_company", {defaultValue: "Requested company"})}</div>
