  ...
  return (
    <Card>
      <CardHeader className="px-5 pt-5 pb-2">
        <CardTitle className="text-base">{t("ppages_Costs.finance_ledger", {defaultValue: "Finance ledger"})}</CardTitle>
        <CardDescription>
    ...
    return [
      {
        ...
        label: (
          <span className="flex items-center gap-1.5">
            <span>{t("ppages_Costs.all_providers", {defaultValue: "All providers"})}</span>
    ...
    return [
      {
        ...
        label: (
          <span className="flex items-center gap-1.5">
            <span>{t("ppages_Costs.all_billers", {defaultValue: "All billers"})}</span>
            {billerKeys.length > 0 ? (
              <>
                <span className="font-mono text-xs text-muted-foreground">{formatTokens(allTokens)}</span>
                <span className="text-xs text-muted-foreground">{formatCents(allCents)}</span>
    ...
    <div className="space-y-6">
      <div className="space-y-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                <h1 className="text-3xl font-semibold tracking-tight">{t("ppages_Costs.costs", {defaultValue: "Costs"})}</h1>
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              label={t("ppages_Costs.inference_spend", {defaultValue: "Inference spend"})}
              value={formatCents(spendData?.summary.spendCents ?? 0)}
              subtitle={`${formatTokens(inferenceTokenTotal)} tokens across request-scoped events`}
              icon={DollarSign}
            />
            <MetricTile
              label={t("ppages_Costs.budget", {defaultValue: "Budget"})}
            ...
            />
            <MetricTile
              label={t("ppages_Costs.finance_net", {defaultValue: "Finance net"})}
            ...
            />
            <MetricTile
              label={t("ppages_Costs.finance_events", {defaultValue: "Finance events"})}
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        <TabsList variant="line" className="justify-start">
          <TabsTrigger value="overview">{t("ppages_Costs.overview", {defaultValue: "Overview"})}</TabsTrigger>
          <TabsTrigger value="budgets">{t("ppages_Costs.budgets", {defaultValue: "Budgets"})}</TabsTrigger>
          <TabsTrigger value="providers">{t("ppages_Costs.providers", {defaultValue: "Providers"})}</TabsTrigger>
          <TabsTrigger value="billers">{t("ppages_Costs.billers", {defaultValue: "Billers"})}</TabsTrigger>
          <TabsTrigger value="finance">{t("ppages_Costs.finance", {defaultValue: "Finance"})}</TabsTrigger>
        </TabsList>

        <TabsContent value="overview" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">{t("ppages_Costs.select_a_start_and_end_date_to_load_data", {defaultValue: "Select a start and end date to load data."})}</p>
          ) : showOverviewLoading ? (
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.3fr,1fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("ppages_Costs.inference_ledger", {defaultValue: "Inference ledger"})}</CardTitle>
                    <CardDescription>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.25fr,0.95fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("ppages_Costs.by_agent", {defaultValue: "By agent"})}</CardTitle>
                    <CardDescription>{t("ppages_Costs.what_each_agent_consumed_in_the_selected_period", {defaultValue: "What each agent consumed in the selected period."})}</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-2 px-5 pb-5 pt-2">
                    {(spendData?.byAgent.length ?? 0) === 0 ? (
                      <p className="text-sm text-muted-foreground">{t("ppages_Costs.no_cost_events_yet", {defaultValue: "No cost events yet."})}</p>
                    ) : (
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("ppages_Costs.by_project", {defaultValue: "By project"})}</CardTitle>
                      <CardDescription>{t("ppages_Costs.run_costs_attributed_through_projectlinked_issues", {defaultValue: "Run costs attributed through project-linked issues."})}</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
                      {(spendData?.byProject.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("ppages_Costs.no_projectattributed_run_costs_yet", {defaultValue: "No project-attributed run costs yet."})}</p>
                      ) : (
            ...
            <>
              <Card className="border-border/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))]">
                <CardHeader className="px-5 pt-5 pb-3">
                  <CardTitle className="text-base">{t("ppages_Costs.budget_control_plane", {defaultValue: "Budget control plane"})}</CardTitle>
                  <CardDescription>
              ...
              {activeBudgetIncidents.length > 0 ? (
                <div className="space-y-3">
                  <div>
                    <h2 className="text-lg font-semibold">{t("ppages_Costs.active_incidents", {defaultValue: "Active incidents"})}</h2>
            ...
            <>
              <Tabs value={effectiveProvider} onValueChange={setActiveProvider}>
                ...
                <TabsContent value="all" className="mt-4">
                  {providers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("ppages_Costs.no_cost_events_in_this_period", {defaultValue: "No cost events in this period."})}</p>
                  ) : (
          ...
          {showCustomPrompt ? (
            ...
            <>
              <Tabs value={effectiveBiller} onValueChange={setActiveBiller}>
                ...
                <TabsContent value="all" className="mt-4">
                  {billers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("ppages_Costs.no_billable_events_in_this_period", {defaultValue: "No billable events in this period."})}</p>
                  ) : (
                    <div className="grid gap-4 md:grid-cols-2">
                      {billers.map((biller) => {
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.2fr,0.95fr]">
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("ppages_Costs.by_biller", {defaultValue: "By biller"})}</CardTitle>
                      <CardDescription>{t("ppages_Costs.accountlevel_financial_events_grouped_by_who_charg", {defaultValue: "Account-level financial events grouped by who charged or credited them."})}</CardDescription>
                    </CardHeader>
                    <CardContent className="grid gap-4 px-5 pb-5 pt-2 md:grid-cols-2">
                      {(financeData?.byBiller.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("ppages_Costs.no_finance_events_yet", {defaultValue: "No finance events yet."})}</p>
                      ) : (
