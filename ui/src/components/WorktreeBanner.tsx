  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 overflow-hidden whitespace-nowrap">
        <span className="shrink-0 opacity-70">{t("pcomponents_WorktreeBanner.worktree", {defaultValue: "Worktree"})}</span>
        <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-current opacity-70" aria-hidden="true" />
        <button
          type="button"
          onClick={handleCopyName}
          title={t("pcomponents_WorktreeBanner.click_to_copy_worktree_name", {defaultValue: "Click to copy worktree name"})}
