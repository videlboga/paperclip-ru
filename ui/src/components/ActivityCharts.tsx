...
export function RunActivityChart(props: RunChartProps) {
  ...
  if (!hasData) return <p className="text-xs text-muted-foreground">{t("pcomponents_ActivityCharts.no_runs_yet", {defaultValue: "No runs yet"})}</p>;
...
export function PriorityChart({ issues }: { issues: { priority: string; createdAt: Date }[] }) {
  ...
  if (!hasData) return <p className="text-xs text-muted-foreground">{t("pcomponents_ActivityCharts.no_issues", {defaultValue: "No issues"})}</p>;
